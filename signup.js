...
function validation(input, inputName, re, label) {
  function Rfalse() {
    input.style =
      "border-bottom:1px solid red;background-image: linear-gradient(0deg, rgba(255,0,0,.3) 0%, rgba(0,0,0,0) 30%);";
  }

  function Rtrue() {
    input.style =
      "border-bottom:1px solid green;background-image: linear-gradient(0deg, rgba(0,255,0,.3) 0%, rgba(0,0,0,0) 30%);";
  }

  function content() {
    swal(
      "Noticeable",
      `The ${inputName} must Contains only letters and numbers for easy memorization`,
      "error"
    );
  }
  ...
  function number(less, more) {
    if (totxt(input).length < less || totxt(input).length > more) {
      swal(
        "Noticeable",
        `the ${inputName} Not less than ${less} and not more than ${more}.`,
        "error"
      );
    } else {
      content();
  ...
  function empty() {
    swal(
      "Noticeable",
      `You should not leave the  ${inputName} field blank .`,
      "error"
    );
  }
    ...
    if (inputName == "username") {
      for (let i = 0; i < dataPro.length; i++) {
        if (dataPro[i].Username == totxt(username)) {
          swal(
            "Noticeable",
            "The account has been created, please choose another",
            "error"
          );
...
function check() {
  ...
  if (
    ...
    username.style =
      "border-bottom:1px solid #fff;background-image: rgba(0,0,0,0);";
    ...
    swal(
      ...
      "The account has been created, please login",
      "success"
    );
...
emailIcon.onclick = function () {
  swal(
    `The email must be valid to communicate with you in case of sending a confirmation code.`
  );
};
